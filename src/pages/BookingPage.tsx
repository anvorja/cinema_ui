...
const BookingPage = () => {
  ...
  // Construir showtime actual con useMemo
  const currentShowtime = useMemo(() => {
    return showtime || getShowtimeById(theaterId, showtimeId) || {
      id: parseInt(showtimeId),
      time: '19:30',
  ...
  // Inicializar booking context (maneja la lógica de contexto)
  const { bookingData } = useBookingInitialization(
    movie,
    theater,
    currentShowtime,
    selectedDate
  ...
  // Estado local para UI (no afecta el contexto hasta continuar)
  const {
    ticketCount,
    handleTicketChange,
    calculatePrices,
  ...
  // Usar datos del contexto si están disponibles, sino datos de navegación
  const currentMovie = movie || bookingData.movie;
  const currentTheater = theater || bookingData.theater;
  const currentDate = selectedDate || bookingData.selectedDate;

  // Calcular precios usando el hook de UI
  const prices = calculatePrices(currentShowtime?.price || 18000);

  const handleContinue = () => {
    // SOLO AQUÍ sincronizamos con el contexto global
    updateBooking({
      ticketCount,
    ...
    navigate('/payment', {
      state: {
        ...
        selectedDate: currentDate,
        ticketCount,
        ticketPrice: currentShowtime?.price || 18000,
              ...
              <GlassCard className="p-6 sticky top-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      <span className="text-white/80">
                        Tarifa de servicio ({ticketCount} x {formatPrice(prices.serviceFee)})
                      </span>
                      <span className="text-white">
                        {formatPrice(prices.totalServiceFees)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-green-400 font-bold text-xl">
                      {formatPrice(prices.totalAmount)}
                    </span>
                  </div>
