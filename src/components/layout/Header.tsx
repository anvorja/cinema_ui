...
// import { GlassCard, PremiumButton, GlassInput } from '../common';
...
const Header = () => {
    ...
    // Handle scroll effect (sin cambios)
    useEffect(() => {
        const handleScroll = () => {
            setIsScrolled(window.scrollY > 20);
        };
    ...
    // Close sidebar when route changes (sin cambios)
    useEffect(() => {
        setIsSidebarOpen(false);
    }, [location]);

    // Close dropdown when clicking outside (ACTUALIZADO para incluir búsqueda)
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (showUserProfile) {
                const dropdown = event.target.closest('.user-profile-dropdown');
            ...
            // NUEVO: Cerrar resultados de búsqueda
    ...
    return (
        <>
            <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
                ...
                >
                    <div className="flex items-center justify-between px-6 py-3">
                        {/* Logo and Hamburger (sin cambios) */}
                        <div className="flex items-center gap-4">
                            {/* Hamburger Menu */}
                            <button
                                onClick={() => {
                        ...
                        {/* Search and User Section (ACTUALIZADO con funcionalidad) */}
                        <div className="flex items-center gap-4">
                            {/* Search - ACTUALIZADO con funcionalidad completa */}
                            ...
                            {/* Search Button Mobile (sin cambios) */}
                            <button className="md:hidden p-2 rounded-lg glass-hover transition-all duration-200 hover:scale-105">
                                <MagnifyingGlassIcon className="w-5 h-5 text-white" />
                            </button>

                            {/* User Profile or Login Button (sin cambios) */}
                            {isAuthenticated ? (
                                <div className="relative">
                                    <button
                                        onClick={handleUserClick}
            ...
            {/* NUEVO: Resultados de búsqueda usando createPortal */}
            ...
            {/* Backdrop for mobile sidebar (sin cambios) */}
            {isSidebarOpen && (
                <div
                    className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 lg:hidden"
                    onClick={() => setIsSidebarOpen(false)}
