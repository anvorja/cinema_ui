...
const Sidebar = ({ isOpen, onClose, onLoginClick, onRegisterClick, ..._rest }: { isOpen: any; onClose: any; onLoginClick: any; onRegisterClick: any; [key: string]: any }) => {
    const { isAuthenticated, logout } = useAuth();
    const [showUserProfile, setShowUserProfile] = useState(false);
    const location = useLocation();

    const menuSections: Array<{ title: string; items: Array<{ name: string; href?: string; icon: any; color: string; type?: string; action?: () => void }> }> = [
        {
            title: 'CINE',
            items: [
                {
